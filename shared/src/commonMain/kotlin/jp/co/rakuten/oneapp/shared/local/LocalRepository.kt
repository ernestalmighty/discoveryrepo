...
class LocalRepository(private val sqlDriver: SqlDriver,
    ...
    fun saveFavorite(item: Item): Unit =
        item.run {
            val result = this.toFavoriteWrapper().toApiFavoriteWrapper()
    ...
    fun saveBookmark(item: Item): Unit =
        item.run {
            val result = this.toBookmarkWrapper().toApiBookmarkWrapper()
            val json = jsonConfiguration.encodeToString(result)
...
private fun Item.CarouselItem.toBookmarkDetails(dateTime: String) = BookmarkDetails(
    title = title,
    imageUrl = imageUrl,
    serviceName = service.title,
    serviceImageUrl = service.imageUrl,
